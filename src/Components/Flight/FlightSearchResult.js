...
const FlightSearchResult = () => {
  ...
  const handleChange = (e) => {
    ...
    if (e.target.checked) {
      const OutARR = currentItems.map((key) => key.FlightDetails.Outbound.filter((item) => item.Carrier !== e.target.value))
      const InARR = currentItems.map((key) => key.FlightDetails.Inbound.filter((item) => item.id !== e.target.value))
      const a = [...OutARR, ...InARR]
      // console.log("pak", a)
      setCheckValue(a)
      setCheckValueInbound(InARR)

      // const filArr = [...currentItems.map((key, index) => (
      // key.FlightDetails.Outbound.filter((item) => item.Carrier === carrier)))]
      // setCheckValue(Object.keys(flights).map((key, index) => (
      //   flights[key].FlightDetails.Inbound.filter((item) => item.Carrier === carrier)
      // )))
      // console.log("=====if")

    }
    else {
      setCheckValue(currentItems)
      setCheckValueInbound(currentItems)
  ...
  // console.log("InARR", checkValueInbound)
                      ...
                      {currentItems?.map((key, index) =>
                          key.FlightDetails?.Outbound?.map((item) => (
                              <div className="filter-list fr-br-botm pt-2" key={item.id}>
                                <ul className="p-0">
                                  <li>
                                    <input type="checkbox" id="" name="airlines" defaultChecked={check} onChange={handleChange} />
